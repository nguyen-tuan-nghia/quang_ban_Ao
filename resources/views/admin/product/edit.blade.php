...
@section('content')
    ...
    <form id="product_creat" action="{{ URL::to('/dashboard/product/update/' . $product->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label>Tên sản phẩm</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                value="{{ $product->name }}" onkeyup="ChangeToSlug();" placeholder="Enter name">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                value="{{ $product->slug }}{{ rand(0, 30) }}" placeholder="Slug">
            @error('slug')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            <label>Mô tả</label>
            <textarea rows="10" cols="50" class="form-control @error('content') is-invalid @enderror" id="content" name="content">
    {{ $product->content }}
    </textarea>
            @error('content')
                <div class="alert alert-danger">{{ $message }}</div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('cost_price') is-invalid @enderror price_format" id="cost_price"
                name="cost_price" value="{{ $product->cost_price }}" placeholder="0">
            @error('cost_price')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="form-group">
            <label>Giá bán</label>
            <input type="text" class="form-control @error('price') is-invalid @enderror price_format" id="price" name="price"
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="category_id">
                <option value="" @if ($product->category_id == null) selected @endif>Không thuộc danh mục nào</option>
                        ...
                        @if ($val->category_parent == 0)
                            ...
                            @foreach ($category as $key2 => $val2)
                                @if ($val2->category_parent == $val->id)
                                    <option @if ($val2->id == $product->category_id) selected @endif value="{{ $val2->id }}">
                                        --{{ $val2->name }}--</option>
                                @endif
                            @endforeach
                        @endif
    ...
    <script>
        $(document).ready(function() {
            var i = 0;
            let preloaded = [];
            <?php foreach($product->gallery as $key => $val) { ?>
            preloaded.push({
                id: "{{ $val->id }}",
                src: "{{ asset('product/thumbnails/' . $val->image) }}"
            });
            <?php } ?>
            ...
            $.fn.imageUploader = function(options) {
                ...
                let createImg = function(src, id) {
                    ...
                    $button.on("click", function(e) {
                        prevent(e);
                        if (window.confirm("Ảnh sẽ bị xóa mãi mãi")) {
                            let index = id;
                            // alert(index);
                                $("input[name='old']").each(function(i, cont) {
                                        if (i > index) {
                                            $(cont).attr("value", i - 1);
                                        }
                                    });
