...
@section('content')
    ...
    @endif
    <form id="product_creat" action="{{ URL::to('/dashboard/post/update').'/'.$post->id }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label>Tên bài viết</label>
            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="name"
                value="{{$post->title}}" onkeyup="ChangeToSlug();" placeholder="Enter name" required>
            @error('title')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                value="{{ $post->slug }}{{ rand(0, 30)}}" placeholder="Slug" required>
            @error('slug')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('keywords') is-invalid @enderror" name="keywords"
                value="{{ $post->keywords }}" data-role="tagsinput" required>
            @error('keywords')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="image-upload">
            <label for="file-input">
                <div class="upload-icon">
                    <img class="icon" src="{{ asset('update/images/images').'/'.$post->image }}">
                </div>
            </label>
    ...
    @enderror
        ...
        <div class="form-group">
            <label>Mô tả</label>
            <textarea class="form-control " name="content" id="editor1" required>{{$post->content}}</textarea>
            @error('content')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
