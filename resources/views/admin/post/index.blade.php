            ...
            <table class="table" id='myTable'>
                ...
                <tbody>
                    ...
                    <tr id="product_{{ $pro->id }}">
                        <td><img style="width:20%" src="{{asset('update/images/images').'/'.$pro->image}}"></td>
                    </td>
                        <td>{{ $pro->title }}</td>
                        <td>{{ $pro->created_at }}</td>
                        <td><a href="{{ url('/dashboard/post/edit/'.$pro->id) }}">Sửa <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
...
<script>
    function del_product(id){
        if(window.confirm("Bạn có chắc muốn xóa bài viết trên ?")){
            $.ajax({
                url:"{{ url('/dashboard/post/delete') }}"+'/'+id,
                method:"GET",
                headers: {
